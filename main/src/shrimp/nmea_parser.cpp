...
static void parse_utc_time(esp_gps_t *esp_gps)
{
    ...
    // printf("RMC Time [%d:%d:%d]\n,",esp_gps->parent.tim.hour,esp_gps->parent.tim.minute,esp_gps->parent.tim.second);
...
static void parse_rmc(esp_gps_t *esp_gps)
{
    ...
    case 6: /* Longitude east(1)/west(-1) information */
        ...
        break;
    case 7: /* Process ground speed in unit m/s  1knt =0.514444 m/s */
        esp_gps->parent.speed = strtof(esp_gps->item_str, NULL) /3.6; //to km/h
...
static void parse_vtg(esp_gps_t *esp_gps)
{
    ...
    case 5:/* Process ground speed in unit m/s */
        esp_gps->parent.speed = strtof(esp_gps->item_str, NULL) / 3.6; //km/h * 0.514444;//knots to m/s
        // esp_gps->parent.speed = strtof(esp_gps->item_str, NULL) * 1.852;//knots to kmh
        break;
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t *config)
{
    ...
    BaseType_t err = xTaskCreate(
                         ...
                         &esp_gps->tsk_hdl);
    gpsH=esp_gps->tsk_hdl;            //to kill it when lat/long is recevied
    if (err != pdTRUE) {
