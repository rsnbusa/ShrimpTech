...
void generic_modbus_task(void *pArg)
{
    ...
    // printf("modbus_sensor->rw: %s %d\n", modbus_sensor->modbus_sensor_name, modbus_sensor->rw);
    if(modbus_sensor->rw>0)
    ...
    // ! DO NOT REMOVE used to give time to the caller to suspend the task before interacting with the modbus rs485 manager
    ...
    while (true)
    {
        ...
        // Wait before next reading cycle refresh per configuration * MINUTES to convert to minutes
        // vTaskDelay(pdMS_TO_TICKS(refresh_rate * MODBUSMINUTES));
        vTaskDelay(pdMS_TO_TICKS(refresh_rate * 10000));
    }
}