...
static bool validate_reset_command(cJSON *resetCommand, char *optionBuffer, size_t bufferSize, ResetLevel *outLevel)
{
    ...
    {
        MESP_LOGE(MESH_TAG, "Reset command validation arguments invalid");
        return false;
    }
    
    if(!cJSON_IsObject(resetCommand))
    {
        MESP_LOGE(MESH_TAG, "Reset command payload invalid");
        return false;
    }
    ...
    if(!optionNode)
    {
        MESP_LOGE(MESH_TAG, "Reset command missing option field");
        return false;
    }
    
    if(!cJSON_IsString(optionNode) || !optionNode->valuestring || strlen(optionNode->valuestring) == 0)
    {
        MESP_LOGE(MESH_TAG, "Reset option parameter invalid or empty");
        return false;
    }
    
    if(strlen(optionNode->valuestring) >= bufferSize)
    {
        MESP_LOGE(MESH_TAG, "Reset option string too long");
        return false;
    }
    ...
    if(level < 0)
    {
        MESP_LOGE(MESH_TAG, "Invalid reset option: %s", optionBuffer);
...
static int execute_reset(ResetLevel resetLevel)
{
    ...
    {
        case RESET_ALL:
            MESP_LOGI(MESH_TAG, "Executing full factory reset");
        ...
        case RESET_METER:
            MESP_LOGI(MESH_TAG, "Resetting meter configuration");
        ...
        case RESET_MESH:
            MESP_LOGI(MESH_TAG, "Resetting mesh configuration");
        ...
        case RESET_COLLECT:
            MESP_LOGW(MESH_TAG, "RESET_COLLECT not implemented");
            break;
            
        case RESET_SEND:
            MESP_LOGW(MESH_TAG, "RESET_SEND not implemented");
            break;
            
        case RESET_MQTT:
            MESP_LOGW(MESH_TAG, "RESET_MQTT not implemented");
            break;
            
        case RESET_CONF:
            MESP_LOGI(MESH_TAG, "Setting reconfigure mode");
        ...
        case RESET_STATUS:
            MESP_LOGI(MESH_TAG, "Setting configuration mode");
        ...
        default:
            MESP_LOGE(MESH_TAG, "Unhandled reset level: %d", resetLevel);
...
int cmdReset(void *argument)
{
    ...
    MESP_LOGI(MESH_TAG, "Reset CMD");
    
    cJSON *resetCommand = (cJSON *)argument;
    
    if(resetCommand == NULL)
    {
        MESP_LOGE(MESH_TAG, "Reset command argument is NULL");
        return ESP_FAIL;
    }
