...
static int initialize_sensor_descriptors(descriptor_array_t *devicesarr, const panel_modbus_specs_t *sensorinfo)
{
    ...
    for (int a = 0; a < MAX_SENSORS; a++)
    {
        ...
        if (label == NULL)
        {
            ESP_LOGE(TAG, "panels_task: Memory allocation failed for param_key label.");
            continue;
        }
        ...
        if (labelunits == NULL)
        {
            ESP_LOGE(TAG, "panels_task: Memory allocation failed for param_units label.");
            free(label);
            continue;
        }
...
// get the descriptors as saved from the web configuration and in theConf structure... thsi type is specific for the panel_modbus_specs_t
